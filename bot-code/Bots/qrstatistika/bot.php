...
$kanal_chat_id = "-1001811837821";
$botModel = Bot::where('bot_id', '5592598131')->first();
...
if($text == "/start"){
  ...
  else
  {
    ...
    $bot_user = BotUser::where('bot_id', $botModel->id)->where('user_id', $chat_id_base)->first();
...
if(($call_data == 'Русский' || $call_data == 'Ўзбек' || $call_data == 'Қарақалпақ') && $map==0){
    ...
    {
        $tg_user->update(['lang' => 'ru'], ['user_id' => $call_chat_id]);
        $user_lang = 'ru';
    }
    if($call_data == 'Ўзбек')
    {
...
if (($photo || $video) && $map=="90") {
    ...
    if ($photo) {
        $file = $photo[0]['file_id'];
        $file_type = "photo";
    }
    if ($video) {
        $file = $video[0]['file_id'];
        $file_type = "video";
    }
